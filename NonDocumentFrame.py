...
import NonDocProcessing as non_doc_text

class NonDocumentFrame (ctk.CTkFrame):
	file_name = "" #available throughout the class
	ocr_result = ""
	def __init__(self, master):
		super().__init__(master)
		self.header= ctk.CTkLabel(self, text = "Non-Document",font=('Times New Roman', 24, 'bold'))
		...
		self.convert_button= ctk.CTkButton(self, text="Convert", command= lambda: cf.convert_button_clicked (self, 2))
		self.convert_button.grid(row = 2, column = 1, padx = 10, pady = (10,0))
		#----------------------------------------
		...
		self.converted_image_frame.grid(row = 4, column = 1, padx = (10,10), pady =(10,0))
		#---------------------------------------------------------------------
		...
		self.text_area.grid(row = 4, column = 2, padx = (10,10), pady = (10, 0))
		#---------------------------------------------------------------------

		#InsideFrame ()
		self.translate_Frame= ctk.CTkFrame (self, width = 420)
		...
		self.voice_Frame = ctk.CTkFrame (self, width= 420)
		self.voice_Frame.grid_propagate (False)
		self.voice_Frame.columnconfigure (1, weight= 1)
		self.voice_Frame.grid (row = 5, column = 1, padx = 23, pady = 10, sticky = 'w')
		# --------------------------------------------------------------------------------

		self.fileSave_Frame = ctk.CTkFrame (self, width= 400)
		...
		self.source_comboBox = ctk.CTkComboBox (master= self, values = non_doc_text.source_language, command = self.combobox_callback_source)
		...
		self.translate_comboBox = ctk.CTkComboBox ( master= self.translate_Frame, 
					     values = translate.lang_value_list, command = self.combobox_callback_translate)
	...
	def check_source_language (self, e): 
		...
		if typed == '':
			data = non_doc_text.source_language
		else:
			data = []
			for item in non_doc_text.source_language:
				if typed.lower() in item.lower():
					data.append (item)
...
class ImageFrame(ctk.CTkFrame):
	def __init__(self, master):
		super().__init__(master)
		self.header= ctk.CTkLabel(self,text = "IMage",font=('Times New Roman', 10, 'bold'), height = 450, width = 400)
